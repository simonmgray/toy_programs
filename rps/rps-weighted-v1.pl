...
####            The R, P, S Game            ####
####                  aka                   ####
#### Simon Gray's better attempt at Bayes   ####
################################################
...
#########
##Subroutine randomthrow: gives a 'random' throw based on the units digit of seconds in time. It's not really random, but the player would not be able to predict the computers throw without knowing the underlying code. The inputs to the code $$ are (P_R2, P_S2) in decimal form (between 0 and 1)
#########
...
sub randomthrow($$){
    ...
    #weight R2 bound by P_R2 probability.
    $R2bound = $P_R2_ref*9;
    
    #weight S2 bound by P_S2 probability
    $S2bound = 9-(9*$P_S2_ref);
...
#Initialize P_R2 and P_S2 to 1/3 for first rounds of game
$P_R2 = 1/3;
$P_P2 = 1/3;
$P_S2 = 1/3;
...
$computer_throw = &randomthrow($P_R2, $P_S2);
...
while($input gt ''){
    if($input eq 'quit'){
        ...
        ##########
        ## Generate predicted human throw by weighted random throw, where weights are based on P_R2, P_P2, and P_S2
        ##########

        $pred_hu_throw = &randomthrow($P_R2, $P_S2);

        ##########
        ## Choose computer throw to beat $pred_hu_throw
        ##########
        
        if($pred_hu_throw eq 'r'){
            $computer_throw = 'p';
        }
        elsif($pred_hu_throw eq 'p'){
            $computer_throw = 's';
        }
        else{
            $computer_throw = 'r'
